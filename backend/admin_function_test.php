...
curl_setopt($ch, CURLOPT_COOKIEJAR, 'admin_function_cookies.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, 'admin_function_cookies.txt');
...
foreach ($adminPages as $pageName => $pageUrl) {
    echo "3. {$pageName}ページをテスト中...\n";
    
    curl_setopt($ch, CURLOPT_URL, 'http://localhost:8000' . $pageUrl);
    ...
    echo "   {$pageName}ページ結果: HTTP {$httpCode}\n";

    if ($httpCode === 200) {
        if (strpos($response, 'EvoX 管理画面') !== false) {
            echo "   ✅ {$pageName}ページアクセス成功\n";
        } else {
            echo "   ⚠️ {$pageName}ページアクセス成功だが、内容に問題がある可能性があります\n";
        }
    } else {
        echo "   ❌ {$pageName}ページアクセス失敗\n";
...
// クリーンアップ
if (file_exists('admin_function_cookies.txt')) {
    unlink('admin_function_cookies.txt');
}

echo "\n=== テスト完了 ===\n";
echo "管理画面機能ページのテストが完了しました。\n";
